...
@Namespace("/back/service/friend/log")
@Results({ @Result(name = "success", type = "freemarker", location = "friendLogList.ftl") })
public class FriendLogAction extends AppListActionSupporter
{

	private static final long serialVersionUID = 8199262629477758758L;

	private static transient final Logger log = LogManager.getLogger(FriendLogAction.class);
	 ...
	 * 靜態收集器
	 */
	protected transient FriendCollector friendCollector = FriendCollector.getInstance();
	 ...
	 * 多筆列表資料
	 */
	private List<FriendLog> friendLogs = new LinkedList<FriendLog>();

	public FriendLogAction()
	{}
	...
	public void initialize()
	{
		...
		//初始化log查詢條件
		initializeInquiry(friendCollector.createLogInquiry());
	}
	...
	public List<FriendLog> getFriendLogs()
	{
		return friendLogs;
	}

	public void setFriendLogs(List<FriendLog> friendLogs)
	{
		this.friendLogs = friendLogs;
	}
	...
	{
		return friendCollector.getActionOptions();
	}
	...
	{
		return friendCollector.getActionName(value, locale);
	}
	...
	{
		friendLogs = friendLogService.findFriendLog(inquiry, siteSeq, userId, clientIp);
	}
	...
	{
		List<FriendLog> result = new LinkedList<FriendLog>();
		int size = 0;
		try
		{
			//String轉成Long
			List<Serializable> buffs = CollectionHelper.toLongs(seqs);
			result = friendLogService.delete(FriendLogImpl.class, buffs);
			size = result.size();
			if (size > 0)
			{
