package org.openyu.cms.vote.log.impl;
...
import org.openyu.cms.vote.po.bridge.ActionTypeBridge;
...
@Table(name = "cms_vote_log")
@SequenceGenerator(name = "cms_vote_log_g", sequenceName = "cms_vote_log_s", allocationSize = 1)
//when use ehcache, config in ehcache.xml
@Cache(usage = CacheConcurrencyStrategy.READ_WRITE, region = "org.openyu.cms.vote.log.impl.VoteLogImpl")
@Proxy(lazy = false)
@org.hibernate.annotations.Table(appliesTo = "cms_vote_log", indexes = { @org.hibernate.annotations.Index(name = "idx_cms_vote_log_1", columnNames = {
		"site_seq", "client_ip", "user_id", "log_date", "action_type" }) })
//--------------------------------------------------
//search
//--------------------------------------------------
//@Indexed
public class VoteLogImpl extends AppLogEntitySupporter implements VoteLog
{
	...
	@Id
	@Column(name = "seq")
	@GeneratedValue(strategy = GenerationType.AUTO, generator = "cms_vote_log_g")
	public Long getSeq()
	{
		return seq;
	}
	...
	@Column(name = "vote_id", length = 255)
	@Field(store = Store.YES, index = Index.YES, analyze = Analyze.NO)
	...
	@Type(type = "org.openyu.cms.vote.po.userType.ActionTypeUserType")
	@Field(store = Store.YES, index = Index.YES, analyze = Analyze.NO)
	...
	public String toString()
	{
		...
		builder.append("voteSeq", voteSeq);
	...
	public Object clone()
	{
		VoteLogImpl copy = null;
		copy = (VoteLogImpl) super.clone();
		//
