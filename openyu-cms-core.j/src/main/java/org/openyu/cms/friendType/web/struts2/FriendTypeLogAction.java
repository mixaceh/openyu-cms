...
@Namespace("/back/service/friendType/log")
@Results({ @Result(name = "success", type = "freemarker", location = "friendTypeLogList.ftl") })
public class FriendTypeLogAction extends AppListActionSupporter
{

	private static final long serialVersionUID = 8199262629477758758L;

	private static transient final Logger log = LogManager.getLogger(FriendTypeLogAction.class);
	 ...
	 * 靜態收集器
	 */
	protected transient FriendTypeCollector friendTypeCollector = FriendTypeCollector.getInstance();
	 ...
	 * 多筆列表資料
	 */
	private List<FriendTypeLog> friendTypeLogs = new LinkedList<FriendTypeLog>();

	public FriendTypeLogAction()
	{}
	...
	public void initialize()
	{
		...
		//初始化log查詢條件
		initializeInquiry(friendTypeCollector.createLogInquiry());
	}
	...
	public List<FriendTypeLog> getFriendTypeLogs()
	{
		return friendTypeLogs;
	}

	public void setFriendTypeLogs(List<FriendTypeLog> friendTypeLogs)
	{
		this.friendTypeLogs = friendTypeLogs;
	}
	...
	{
		return friendTypeCollector.getActionOptions();
	}
	...
	{
		return friendTypeCollector.getActionName(value, locale);
	}
	...
	{
		friendTypeLogs = friendTypeLogService.findFriendTypeLog(inquiry, siteSeq, userId, clientIp);
	}
	...
	{
		List<FriendTypeLog> result = new LinkedList<FriendTypeLog>();
		int size = 0;
		try
		{
			//String轉成Long
			List<Serializable> buffs = CollectionHelper.toLongs(seqs);
			result = friendTypeLogService.delete(FriendTypeLogImpl.class, buffs);
			size = result.size();
			if (size > 0)
			{
