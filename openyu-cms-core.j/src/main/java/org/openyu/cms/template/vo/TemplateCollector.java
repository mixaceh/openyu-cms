...
// jaxb
// --------------------------------------------------
@XmlRootElement(name = "templateCollector")
@XmlAccessorType(XmlAccessType.FIELD)
public class TemplateCollector extends BaseCollectorSupporter {

	private static final long serialVersionUID = -1059982996489996588L;

	private static TemplateCollector templateCollector;
	...
	public TemplateCollector() {
		setId(TemplateCollector.class.getName());
	}

	public synchronized static TemplateCollector getInstance() {
		return getInstance(true);
	}

	public synchronized static TemplateCollector getInstance(boolean initial) {
		if (templateCollector == null) {
			templateCollector = new TemplateCollector();
			if (initial) {
				templateCollector.initialize();
			}
			// 此有系統預設值,只是為了轉出xml,並非給企劃編輯用
			templateCollector.actionTypes = EnumHelper
	...
	public void initialize() {
		if (!templateCollector.isInitialized()) {
			templateCollector = readFromSer(TemplateCollector.class);
			// 此時有可能會沒有ser檔案,或舊的結構造成ex,只要再轉出一次ser,覆蓋原本ser即可
			if (templateCollector == null) {
				templateCollector = new TemplateCollector();
			}
			//
			templateCollector.setInitialized(true);
		}
	}
