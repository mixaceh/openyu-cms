 ...
 * 刪除檔案攔截器
 */
public class TemplateDeleteFileInterceptor extends AppMethodInterceptorSupporter
{

	private static transient final Logger log = LogManager
			.getLogger(TemplateDeleteFileInterceptor.class);

	@Autowired
	@Qualifier("templateLogService")
	protected transient TemplateLogService templateLogService;

	public TemplateDeleteFileInterceptor()
	{}
		...
		try
		{
			...
			if (CollectionHelper.notEmpty(ret))
			{
				for (String name : ret)
				{
					...
					if (actionType != null)
					{
						templateLogService.recordChangeTemplate(user, actionType, name, null);
					}
				}
			}
		}
