...
public class ContextItemsUserType extends BaseUserTypeSupporter {
	 ...
	 * @return
	 */
	public String assembleBy_1(Set<ContextItem> src) {
		StringBuilder result = new StringBuilder();
		result.append((src != null ? src.size() : 0));// 0
		for (ContextItem entry : src) {
	 ...
	 * v1 由欄位反組成物件
	 */
	public Set<ContextItem> disassembleBy_1(StringBuilder src) {
		Set<ContextItem> result = new LinkedHashSet<ContextItem>();
		if (src == null) {
			return result;
		}
		...
		for (int i = 0; i < size; i++) {
			...
			int edx = 0;
			ContextItem entry = new ContextItemImpl();
			//
