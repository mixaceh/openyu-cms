...
<body>
	...
	<div id="centercontent">
		<#-- breadcrumb -->
		<div id="breadcrumb">
			<@s.url var="parentUrl" namespace="/back/service/sensitivity/log" action="index" />
			<@s.set name="parentTitle">
				<@s.text name="sensitivity.log.title" />
			</@s.set>
			<@s.url var="currentUrl" namespace="/back/service/sensitivity/log" action="list">
			</@s.url>
		...
		<div class="feature">
			...
			<@s.form id="searchForm" name="searchForm">
					...
					<tbody>
						<tr>
							...
							<td width="63%"><@s.url var="url"
									namespace="/back/service/sensitivity/log" action="find" /> <@s.a href="#"
				...
				<div id="pagination">
					<@s.url var="url" namespace="/back/service/sensitivity/log" action="find" />
		...
		<div id="opnav">
			<ul>
				<#-- delete -->
				<li><@s.url var="url" namespace="/back/service/sensitivity/log"
				...
				<table class="list" width="100%" border="0">
					<thead>
						<tr>
							...
							<th width="5%"><@s.text name="log.orig" />:<@s.text name="sensitivity.dictionarys.key" /></th>
							<th width="6%"><@s.text name="log.orig" />:<@s.text name="sensitivity.dictionarys.value" /></th>
							<th width="4%"><@s.text name="log.orig" />:<@s.text name="sensitivity.dictionarys.valid" /></th>
							<th width="5%"><@s.text name="log.new" />:<@s.text name="sensitivity.dictionarys.key" /></th>
							<th width="6%"><@s.text name="log.new" />:<@s.text name="sensitivity.dictionarys.value" /></th>
							<th width="4%"><@s.text name="log.new" />:<@s.text name="sensitivity.dictionarys.valid" /></th>
						</tr>
					</thead>
					<tbody>
						<@s.iterator value="sensitivityLogs" var="sensitivityLog" status="st">
							<tr>
								<#-- pick -->
								...
								<td><@s.property value="beforeDictionary.key" /></td>
								<td><@s.property value="beforeDictionary.value" /></td>
								<td><@s.property value="%{getTrueFalseName(beforeDictionary.valid)}" /></td>
								<td><@s.property value="afterDictionary.key" /></td>
								<td><@s.property value="afterDictionary.value" /></td>
								<td><@s.property value="%{getTrueFalseName(afterDictionary.valid)}" /></td>
							</tr>
						</@s.iterator>
					</tbody>
				</table>
