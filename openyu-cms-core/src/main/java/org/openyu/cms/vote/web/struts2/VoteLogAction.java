...
@Namespace("/back/service/vote/log")
@Results({ @Result(name = "success", type = "freemarker", location = "voteLogList.ftl") })
public class VoteLogAction extends AppListActionSupporter
{

	private static final long serialVersionUID = 8199262629477758758L;

	private static transient final Logger log = LogManager.getLogger(VoteLogAction.class);
	 ...
	 * 靜態收集器
	 */
	protected transient VoteCollector voteCollector = VoteCollector.getInstance();
	 ...
	 * 多筆列表資料
	 */
	private List<VoteLog> voteLogs = new LinkedList<VoteLog>();

	public VoteLogAction()
	{}
	...
	public void initialize()
	{
		...
		//初始化log查詢條件
		initializeInquiry(voteCollector.createLogInquiry());
	}
	...
	public void setVoteLogs(List<VoteLog> voteLogs)
	{
	...
	{
		return voteCollector.getActionOptions();
	}
	...
	{
		return voteCollector.getActionName(value, locale);
	}
	...
	{
		voteLogs = voteLogService.findVoteLog(inquiry, siteSeq, userId, clientIp);
	}
	...
	{
		List<VoteLog> result = new LinkedList<VoteLog>();
		int size = 0;
		try
		{
			//String轉成Long
			List<Serializable> buffs = CollectionHelper.toLongs(seqs);
			result = voteLogService.delete(VoteLogImpl.class, buffs);
			size = result.size();
			if (size > 0)
			{
