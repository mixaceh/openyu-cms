 ...
 * 儲存檔案攔截器
 */
public class ResourceSaveFileInterceptor extends AppMethodInterceptorSupporter
{

	private static transient final Logger log = LogManager
			.getLogger(ResourceSaveFileInterceptor.class);

	@Autowired
	@Qualifier("resourceLogService")
	protected transient ResourceLogService resourceLogService;

	public ResourceSaveFileInterceptor()
	{}
							...
							Object[] args)
	{
		...
		try
		{
			...
			if (ret)
			{
				resourceLogService.recordChangeResource(user, ActionType.SAVE_FILE, origName,
					origName);
			}
		}
		catch (Throwable ex)
