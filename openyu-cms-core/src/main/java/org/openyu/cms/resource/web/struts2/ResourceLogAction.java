...
import org.openyu.cms.resource.vo.ResourceCollector;
...
@Namespace("/back/service/resource/log")
@Results({ @Result(name = "success", type = "freemarker", location = "resourceLogList.ftl") })
public class ResourceLogAction extends AppListActionSupporter
{

	private static final long serialVersionUID = 8199262629477758758L;

	private static transient final Logger log = LogManager.getLogger(ResourceLogAction.class);
	 ...
	 * 靜態收集器
	 */
	protected transient ResourceCollector resourceCollector = ResourceCollector.getInstance();
	 ...
	 * 多筆列表資料
	 */
	private List<ResourceLog> resourceLogs = new LinkedList<ResourceLog>();

	public ResourceLogAction()
	{}
	...
	public void initialize()
	{
		...
		//初始化log查詢條件
		initializeInquiry(resourceCollector.createLogInquiry());
	}
	...
	{
		return resourceCollector.getActionOptions();
	}
	...
	{
		return resourceCollector.getActionName(value, locale);
	}
	...
	{
		resourceLogs = resourceLogService.findResourceLog(inquiry, siteSeq, userId, clientIp);
	}
	...
	{
		List<ResourceLog> result = new LinkedList<ResourceLog>();
		int size = 0;
		try
		{
			//String轉成Long
			List<Serializable> buffs = CollectionHelper.toLongs(seqs);
			result = resourceLogService.delete(ResourceLogImpl.class, buffs);
			size = result.size();
			if (size > 0)
			{
