...
public class ContextTypeServiceImplTest extends ContextTypeTestSupporter
{
	/**
	 * 隨機產生本文類型資料
	 * 
	 * @return
	 */
	public static ContextType randomContextType()
	{
		...
		//
		ContextType result = new ContextTypeImpl();
		result.setId(ID);
	 ...
	 * @param actual
	 */
	public static void assertContextType(ContextType expected, ContextType actual)
	{
	...
	public void findContextType()
	{
		...
		List<ContextType> result = null;
		int count = 1;

		long beg = System.currentTimeMillis();
		for (int i = 0; i < count; i++)
		{
			result = contextTypeService.findContextType(true);
		}
	...
	public void findContextTypeByInquiry()
	{
		...
		for (int i = 0; i < count; i++)
		{
			result = contextTypeService.findContextType(inquiry, Locale.TRADITIONAL_CHINESE,
				searcher);
		}
		long end = System.currentTimeMillis();
		System.out.println(count + " times: " + (end - beg) + " mills. ");
